...
* No Python: You cannot run tests or any other Python code. Do not use any of the following commands: python, pytest, mypy, pip, apt, apt-get.
                ...
                'properties': {
                    'command': {
                        ...
                        'description': refine_prompt(
                            'The bash command to execute. Do not use any of the following commands: python, pytest, mypy, pip, apt, apt-get. Can be empty string to view additional logs when previous exit code is `-1`. Can be `C-c` (Ctrl+C) to interrupt the currently running process. Note: You can only execute one bash command at a time. If you need to run multiple commands sequentially, you can use `&&` or `;` to chain them together.'
                        ),
                    },
                    'is_input': {
                        'type': 'string',
