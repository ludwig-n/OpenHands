...
* No code execution: You cannot write, compile or run tests or any other code. You may only use bash commands from the <allowed_commands> list.
                ...
                'properties': {
                    'command': {
                        ...
                        'description': refine_prompt(
                            'The bash command to execute. Must be one of the following: <allowed_commands> cd, grep, head, find, rm, git, ls, tail, echo, cat, xargs, pwd, mkdir, which, timeout, sed, wc, C-c, mv, chmod, export, cp, true, pkill, sleep, sort, awk, od, ps, printf, xxd, touch, netstat, lsof, diff, kill, ss, curl, hexdump, tr, file, fuser, nohup, sudo, uniq, sha256sum, basename, cut, tar, wget, man </allowed_commands>. Can be empty string to view additional logs when previous exit code is `-1`. Can be `C-c` (Ctrl+C) to interrupt the currently running process. Note: You can only execute one bash command at a time. If you need to run multiple commands sequentially, you can use `&&` or `;` to chain them together. In this case, make sure that each of these commands is on the <allowed_commands> list.'
                        ),
                    },
                    'is_input': {
                        'type': 'string',
